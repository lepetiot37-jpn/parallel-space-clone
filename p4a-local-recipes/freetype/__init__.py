"""
Surcharge locale de la recette python-for-android pour freetype.

Le serveur officiel download.savannah.gnu.org renvoie fréquemment des
erreurs HTTP 502 depuis les runners GitHub Actions, ce qui fait échouer
la compilation avant même de commencer. Cette version est identique à
la recette upstream, seule l'URL de téléchargement change pour pointer
vers le miroir SourceForge, qui héberge exactement les mêmes archives.

Référence de la recette d'origine :
https://github.com/kivy/python-for-android/blob/master/pythonforandroid/recipes/freetype/__init__.py
"""
...
class FreetypeRecipe(Recipe):
    version = '2.14.1'
    # URL d'origine (peu fiable depuis les runners CI) :
    #   https://download.savannah.gnu.org/releases/freetype/freetype-{version}.tar.gz
    # Miroir SourceForge utilisé à la place :
